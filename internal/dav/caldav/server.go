...
// ResourceTypeResolver lets a Backend classify request paths itself, instead of
// counting the segments after the prefix, which cannot separate a principal
// from a calendar home set when both sit at the same depth.
...
func (b *backend) Put(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return internal.HTTPErrorf(http.StatusRequestEntityTooLarge, "caldav: request body too large: %v", err)
	}
	...
	if _, err := ical.NewDecoder(bytes.NewReader(raw)).Decode(); err != nil {
	...
	co, err := b.Backend.PutCalendarObject(r.Context(), r.URL.Path, raw, &opts)
	if err != nil {
		return err
	}
